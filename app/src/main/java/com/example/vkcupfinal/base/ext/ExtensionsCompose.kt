...
@Composable
fun Modifier.appClickable(withRipple: Boolean = true, onClick: () -> Unit) = this.then(
    clickable(
...
fun Modifier.appShadow(corner: Dp = AppTheme.dimens.x3) = this.then(
    appShadow(RoundedCornerShape(corner))
)


@Composable
fun Modifier.appShadow(shape: Shape) = this.then(
    shadow(
...
val emptyPainter: Painter
    get() = ColorPainter(AppTheme.color.Transparent)
